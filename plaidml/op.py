...
class Equal_ArgMax(tile.Operation):
    def __init__(self, lhs, rhs):
        ...
        super(Equal_ArgMax, self).__init__('function (I) -> (O) { O = 0 < I; }',
...
def equal(lhs, rhs):
    ...
    if (lhs.source and isinstance(lhs.source.op, ArgMax) and
            rhs.source and isinstance(rhs.source.op, ArgMax)):
...
class IsMax(tile.Operation):
    ...
    def __init__(self, value, axes):
        ...
        super(IsMax, self).__init__(code,
                                    [('I', value)],
    ...
    def __init__(self, a, b):
        ...
        if a_ndims == 1:
            ...
            if b_ndims == 1:
                ...
                c_dims = tuple(list(tile.broadcast_dims(a.shape.dims[:-2], b.shape.dims[:-2])) +
...
class Reshape(tile.Operation):
    ...
    def __init__(self, x, dims):
        super(Reshape, self).__init__(
            'function (I) -> (O) {{ O = reshape(I, {}); }}'.format(
                ', '.join([str(d) for d in dims])),
            [('I', x)], [('O', tile.Shape(x.shape.dtype, dims))]
        )
...
class Summation(tile.Operation):
    ...
    def __init__(self, value, axes, keepdims=False):
        ...
        super(Summation, self).__init__(code,
                                        [('I', value)],
