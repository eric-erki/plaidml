...
class SharedBuffer final : public Buffer {
 public:
  ...
  std::shared_ptr<hal::Event> Unmap() final;
...
std::shared_ptr<hal::Event> SharedBuffer::Unmap() {
  return std::make_shared<Event>(arena_->device_state()->cl_ctx(), CLObj<cl_event>());
}
...
void Executor::InitSharedMemory() {
  for (auto cap : device_state_->info().svm_capability()) {
    if (cap != proto::SvmCapability::FineGrainBuffer && cap != proto::SvmCapability::FineGrainSystem) {
      continue;
    }
